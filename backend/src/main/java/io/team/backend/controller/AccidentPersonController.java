...
public class AccidentPersonController {
    ...
    public ResponseEntity<PersonResponse> postInjured(@RequestBody InjuredPersonRequest personRequest) {
        PersonResponse person = personService.createInjuredPerson(personRequest);
        return ResponseEntity.ok(person);
    }

    @PostMapping(value = "/proxy")
    public ResponseEntity<PersonResponse> postProxy(@RequestBody ProxyPersonRequest personRequest) {
